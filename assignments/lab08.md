...
3. **Build xv6**
   ...
   Now you can build xv6 by running `make` in the `xv6-riscv` directory. This will compile and link all the programs, including your new `hello` program.
...
3. **Add the System Call to the System Call Table**
   ...
   Then, you need to add the system call to the system call table. Open the `kernel/syscall.c` and add an `extern` for `sys_kmemfree` after the existing extern definitions.
    ...
    ```
4. **Add kmemfree() to user/usys.pl**
...
7. **Add the User Program to the Makefile**
